...
@app.command()
def dashboard(
    port: int = typer.Option(18790, "--port", "-p", help="Dashboard port"),
    host: str = typer.Option("0.0.0.0", "--host", "-h", help="Dashboard host"),
):
    """Start the nanobot dashboard server."""
    import subprocess
    import sys
    ...
    console.print(f"{__logo__} Starting Nanobot Dashboard\n")
    console.print(f"📊 Dashboard will be available at: [cyan]http://{host}:{port}/dashboard[/cyan]")
    console.print(f"📂 Data directory: [dim]{workspace}/dashboard[/dim]")
    console.print()

    # Start the dashboard server
    dashboard_script = Path(__file__).parent.parent / "skills" / "dashboard" / "server.py"

    if not dashboard_script.exists():
        console.print("[red]Error: Dashboard server not found[/red]")
        console.print(f"Expected at: {dashboard_script}")
        return

    # Run with uvicorn
    try:
        console.print("🚀 Starting server...")
        subprocess.run([sys.executable, str(dashboard_script), str(workspace)])
    except KeyboardInterrupt:
        console.print("\n👋 Dashboard stopped")
